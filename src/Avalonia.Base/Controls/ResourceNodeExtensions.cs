...
namespace Avalonia.Controls
{
    public static class ResourceNodeExtensions
    {
        ...
        private class ResourceObservable : LightweightObservableBase<object?>
        {
            ...
            {
                if (e.Property == StyledElement.ThemeVariantProperty)
                {
                    PublishNext(GetValue());
                }
            }
            ...
            private object? GetValue()
            {
                if (_target is not StyledElement themeStyleable
                    || !_target.TryFindResource(_key, themeStyleable.GetEffectiveThemeVariant(), out var value))
                {
        ...
        private class FloatingResourceObservable : LightweightObservableBase<object?>
        {
            ...
            private object? GetValue()
            {
                if (!(_target.Owner is StyledElement themeStyleable)
                    || !_target.Owner.TryFindResource(_key, themeStyleable.GetEffectiveThemeVariant(), out var value))
                {
